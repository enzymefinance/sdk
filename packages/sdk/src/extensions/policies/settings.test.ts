import { encodeMinMaxInvestmentPolicySettings } from "./instances/minMaxInvestmentPolicy.js";
import { decodePolicySettings, encodePolicySettings, policySettingsEncoding } from "./settings.js";
import { encodeAbiParameters } from "viem";
import { expect, test } from "vitest";

test("policySettingsEncoding should contain the correct properties", () => {
  expect(policySettingsEncoding).toMatchInlineSnapshot(`
        [
          {
...
test("decodePolicySettings should throw error if encoded addresses and settings have different lengths", () => {
  ...
  const encoded = encodeAbiParameters(policySettingsEncoding, [addresses, settings]);
